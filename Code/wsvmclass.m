function [xsup,w,d,pos,timeps,alpha,obj]=wsvmclass(x,y,Rweight, c,lambda,kernel,kerneloption,verbose,span, alphainit)
...
% Training set
%      x  		: input data 
%      y  		: output data (treatment label: -/+ 1)
%      Rweight        : R/pi
% parameters
...
if nargin< 10   % nargin: number of function input arguments
    alphainit=[];
end;

if nargin < 9 | isempty(span)
    A = y;
    b = 0;
else
    if span==1
...
H =ps.*(y*y'); 
e = ones(size(y));
C=Rweight*c;  % add subject-specific weight to tuning parameter C
...
if isinf(c)                                                           
    ...
    % change small c to big C (tuning parameter c*sample weight)
