function [Sigma,Alpsup,w0,pos,history,obj,status] = wmklsvm(K,yapp,Rweightapp, c,option,verbose)

% USAGE : weighted mutiple kernel svm
%
% Inputs
%
...
if ~isfield(option,'sigmainit');
    Sigma=ones(1,nbkernel)/nbkernel;  % kernel weights
else
...
switch option.algo
    case 'svmclass'
        [xsup,Alpsup,w0,pos,aux,aux,obj] = wsvmclass([],yapp, Rweightapp,c,lambdareg,kernel,kerneloption,verbosesvm,span,alphainit);
        [grad] = gradwsvmclass(K,pos,Alpsup,yapp);
    case 'svmreg'
...
while loop & nloopmax >0 ;
    ...
    %-----------------------------------------
    t = cputime ;
    [Sigma,Alpsup,w0,pos,obj] = mklwsvmupdate(K,Sigma,pos,Alpsup,w0,c,yapp, Rweightapp, grad,obj,option) ;
    %-----------------------------------------
    % Thresholding
    %-----------------------------------------
