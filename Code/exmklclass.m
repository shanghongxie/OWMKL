...
for i=1: nbiter
    ...
    tic
    [beta,w,b,posw,story(i),obj(i)] = mklsvm(K,yapp,C,option,verbose);
    timelasso(i)=toc

    Kt=mklkernel(xtest,InfoKernel,Weight,option,xapp(posw,:),beta);
    ypred=Kt*w+b;

    bc(i)=mean(sign(ypred)==ytest) . % 0.9148

end;%

Rweightapp=repmat(1,175,1);
option.sumbeta='storefullsum';
[beta,w,b,posw,story(2),obj(2)] = wmklsvm(K,yapp,Rweightapp,C,option,verbose);
    timelasso(2)=toc;
